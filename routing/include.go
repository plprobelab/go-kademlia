...
type check[K kad.Key[K], N kad.NodeID[K], I any] struct {
	NodeInfo kad.NodeInfo[K, N, I]
	Started  time.Time
}

type Include[K kad.Key[K], N kad.NodeID[K], I any] struct {
	rt kad.RoutingTable[K, N]

	// checks is an index of checks in progress
	checks map[string]check[K, N, I]

	candidates *nodeQueue[K, N, I]
...
func NewInclude[K kad.Key[K], N kad.NodeID[K], I any](rt kad.RoutingTable[K, N], cfg *IncludeConfig) (*Include[K, N, I], error) {
	if cfg == nil {
	...
	return &Include[K, N, I]{
		candidates: newNodeQueue[K, N, I](cfg.QueueCapacity),
		cfg:        *cfg,
		rt:         rt,
		checks:     make(map[string]check[K, N, I], cfg.Concurrency),
	}, nil
}

// Advance advances the state of the include state machine by attempting to advance its query if running.
func (b *Include[K, N, I]) Advance(ctx context.Context, ev IncludeEvent) IncludeState {
	ctx, span := util.StartSpan(ctx, "Include.Advance")
	defer span.End()

	switch tev := ev.(type) {

	case *EventIncludeAddCandidate[K, N, I]:
	...
	case *EventIncludeMessageResponse[K, N, I]:
		...
		if ok {
			...
			if tev.Response != nil && len(tev.Response.CloserNodes()) > 0 {
				if b.rt.AddNode(tev.NodeInfo.ID) {
					return &StateIncludeRoutingUpdated[K, N, I]{
						NodeInfo: ch.NodeInfo,
					}
				}
			}
		}
	case *EventIncludeMessageFailure[K, N, I]:
		delete(b.checks, key.HexString(tev.NodeInfo.ID.Key()))
	...
	b.checks[key.HexString(candidate.ID.Key())] = check[K, N, I]{
		NodeInfo: candidate,
		Started:  b.cfg.Clock.Now(),
	}

	// Ask the node to find itself
	return &StateIncludeFindNodeMessage[K, N, I]{
		NodeInfo: candidate,
	}
}

// nodeQueue is a bounded queue of unique NodeIDs
type nodeQueue[K kad.Key[K], N kad.NodeID[K], I any] struct {
	capacity int
	nodes    []kad.NodeInfo[K, N, I]
	keys     map[string]struct{}
}

func newNodeQueue[K kad.Key[K], N kad.NodeID[K], I any](capacity int) *nodeQueue[K, N, I] {
	return &nodeQueue[K, N, I]{
		capacity: capacity,
		nodes:    make([]kad.NodeInfo[K, N, I], 0, capacity),
...
func (q *nodeQueue[K, N, I]) Enqueue(ctx context.Context, n kad.NodeInfo[K, N, I]) bool {
...
func (q *nodeQueue[K, N, I]) Dequeue(ctx context.Context) (kad.NodeInfo[K, N, I], bool) {
	if len(q.nodes) == 0 {
		var v kad.NodeInfo[K, N, I]
		return v, false
	}

	var n kad.NodeInfo[K, N, I]
...
func (q *nodeQueue[K, N, I]) HasCapacity() bool {
...
type StateIncludeFindNodeMessage[K kad.Key[K], N kad.NodeID[K], I any] struct {
	NodeInfo kad.NodeInfo[K, N, I] // the node to send the mssage to
}
...
type StateIncludeRoutingUpdated[K kad.Key[K], N kad.NodeID[K], I any] struct {
	NodeInfo kad.NodeInfo[K, N, I]
}

// includeState() ensures that only Include states can be assigned to an IncludeState.
func (*StateIncludeFindNodeMessage[K, N, I]) includeState() {}
...
func (*StateIncludeRoutingUpdated[K, N, I]) includeState()  {}
...
type EventIncludeAddCandidate[K kad.Key[K], N kad.NodeID[K], I any] struct {
	NodeInfo kad.NodeInfo[K, N, I] // the candidate node
}

// EventIncludeMessageResponse notifies a include that a sent message has received a successful response.
type EventIncludeMessageResponse[K kad.Key[K], N kad.NodeID[K], I any] struct {
	NodeInfo kad.NodeInfo[K, N, I] // the node the message was sent to
...
type EventIncludeMessageFailure[K kad.Key[K], N kad.NodeID[K], I any] struct {
	NodeInfo kad.NodeInfo[K, N, I] // the node the message was sent to
...
func (*EventIncludeAddCandidate[K, N, I]) includeEvent()    {}
func (*EventIncludeMessageResponse[K, N, I]) includeEvent() {}
func (*EventIncludeMessageFailure[K, N, I]) includeEvent()  {}
