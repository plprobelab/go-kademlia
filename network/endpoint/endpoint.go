...
type ResponseHandlerFn[K kad.Key[K], N kad.NodeID[K], I any] func(context.Context, kad.Response[K, N, I], error)

// Endpoint defines how Kademlia nodes interacts with each other.
type Endpoint[K kad.Key[K], N kad.NodeID[K], I any] interface {
	...
	MaybeAddToPeerstore(context.Context, kad.NodeInfo[K, N, I], time.Duration) error
...
// peers.
type ServerEndpoint[K kad.Key[K], N kad.NodeID[K], I any] interface {
	Endpoint[K, N, I]
...
// known remote peers.
type NetworkedEndpoint[K kad.Key[K], N kad.NodeID[K], I any] interface {
	Endpoint[K, N, I]
	// Connectedness returns the connectedness of the given peer.
	Connectedness(N) (Connectedness, error)
}
